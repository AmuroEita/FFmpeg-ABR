...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    {
        av_log(NULL, AV_LOG_ERROR, "Test Option %s not found.\n", t->key);
...
static int read_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        if (abr && pkt_ts >= (stream_start_time + abr_init_duration))
        {
            ...
            if ((pkt->stream_index == is->audio_stream || abr_check_list(is->abr_list, AVMEDIA_TYPE_AUDIO, pkt->stream_index)) && pkt_in_play_range)
            {
                packet_queue_put(&is->audioq, pkt);
            }
            else if ((pkt->stream_index == is->video_stream || (is->video_stream != -1 && abr_check_list(is->abr_list, AVMEDIA_TYPE_VIDEO, pkt->stream_index))) && pkt_in_play_range && !(is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC))
            {
                packet_queue_put(&is->videoq, pkt);
            }
            else if ((pkt->stream_index == is->subtitle_stream || abr_check_list(is->abr_list, AVMEDIA_TYPE_SUBTITLE, pkt->stream_index)) && pkt_in_play_range)
            {
